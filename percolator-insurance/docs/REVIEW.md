...
- [x] **MEDIUM** Close the accrual gaming windows — *DONE (hybrid TWA)*: a global `cum_system_index` accumulator (funding-rate pattern) integrates the account-independent system factors over real elapsed time, advanced by every op + a public `accrue()` keeper hook, so a trader cannot game the rate by timing their touches; per-account leverage is charged at the max of interval endpoints (anti-flicker). Spec: `docs/superpowers/specs/2026-06-17-twa-hybrid-accrual-design.md`. Residual: crowding *side* still point-sampled; sampling density = touch + crank cadence.
